...
void DoEditCS()
{
  ...
  if(fexists(MCFilePluginsPath("ColEdit.elf")))
...
void S_ZipOpenFile(char* ext, int isAltOpen)
{
 ...
 if (fexists(szTempFilePath) && !isdir2(szTempFilePath, &err))
...
void DeleteTempFiles()
{
 ...
 while(itm)
 {
  if (fexists(itm->full) && !isdir2(itm->full, &err))
...
int IsCurFile(void)
{
	...
	if (file && pathbuf && strlen(file->sname))
	{
		if (file->attr & FA_DIRECTORY) return 2;
		return 1;
	}
	else return 0;
}
