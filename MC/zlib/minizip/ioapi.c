...
void clear_read_buf(IO_FILE* pFile)
{
	pFile->buflen = 0;
	pFile->bufpos = 0;
}
...
voidpf fopen_file_func (
   ...
   int mode)
{
	IO_FILE* pFile = malloc(sizeof(IO_FILE));
	zeromem(pFile, sizeof(IO_FILE));
	...
	if (filename && pFile)
	{
		pFile->handle = _open(filename, flags, fmode, &pFile->err);
   ...
   uLong size)
{
	...
	if (pFile)
	{
		...
		if (size > 0)
		{
			...
			{
				pFile->buflen = (uLong)_read(pFile->handle, pFile->readbuf,
			...
			{
				ret += (uLong)_read(pFile->handle, buf, (size_t)size, &pFile->err);
			}
		}
	}
	return ret;
   ...
   uLong size)
{
	...
	if (pFile)
	{
		ret = (uLong)_write(pFile->handle, buf, (size_t)size, &pFile->err);
		clear_read_buf(pFile);
	}
	return ret;
}
   ...
   voidpf stream)
{
	...
	if (pFile)
	{
		ret = _lseek(pFile->handle, 0, S_CUR, &pFile->err, &pFile->err);
		clear_read_buf(pFile);
	}
   ...
   int origin)
{
	...
	if (pFile)
	{
		...
		_lseek(pFile->handle, offset, fseek_origin, &pFile->err, &pFile->err);
		ret = 0;
		clear_read_buf(pFile);
	}
	return ret;
