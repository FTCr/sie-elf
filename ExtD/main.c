...
static const char extfile[]="?:\\etc\\extension.cfg";
...
int main(const char *exename)
{
  ...
  if ((f=_open(extfile,A_ReadOnly,P_READ,&ul))!=-1)
  {
    size_cfg=_lseek(f,0,S_END,&ul,&ul);
    _lseek(f,0,S_SET,&ul,&ul);
    s=CFG=malloc(size_cfg+1);
    if (s)
    {
      s[_read(f,s,size_cfg,&ul)]=0;
